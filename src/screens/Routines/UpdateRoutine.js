...
export default function UpdateRoutine({navigation,route:{params}}) {
    ...
    const [sunday,setSunday] = useState(params.sunday);
    const [monday,setMonday] = useState(params.monday);
    const [tuesday,setTuesday] = useState(params.tuesday);
    const [wedensday,setWedensday] = useState(params.wedensday);
    const [thursday,setThursday] = useState(params.thursday);
    const [friday,setFriday] = useState(params.friday);
    const [saturday,setSaturday] = useState(params.saturday);
    ...
    const handleTime = (event, selectedDate) => {
      ...
      let handleSelectedDayStyle = (day) => {
       ...
       const handleUpdate = async() => {
        ...
        var response = await HttpRequest("/Routines/EditRoutineById","post",{Id:params.id,Title:title,UserId:params,TagId:selectedTag,StartHour:time.getTime(),Sunday:sunday,Monday:monday,Tuesday:tuesday,Wedensday:wedensday,Thursday:thursday,Friday:friday,Saturday:saturday,IsActive:isActive})
    ...
    let handleDelete = async() =>{
        ...
        dispatch(setApp({isLoading:true}));
        var response = await HttpRequest(`/Routines/DeleteRoutineById/${params.id}`,"post");
